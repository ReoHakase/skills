...
Priorityは重要度。0が最低、3が最高。P1とP2が通常作業の大半を占める。
...
P3は少数に保つ。常時P3が多い場合、計画か品質ゲートが壊れている。
...
S3をそのまま実装するのは例外。先にcontract issue、spike、sub-issue分割を検討する。
...
```text
max(Complexity, Risk) <= 1 かつ Size <= 1 -> agent:fast
max(Complexity, Risk) == 2 または Size == 2 -> agent:standard
max(Complexity, Risk) == 3 または Size == 3 -> agent:frontier候補
```

ただし、SizeだけがS3でComplexity/Riskが低い場合は、frontier agentへ投げるよりIssue分割を優先する。
