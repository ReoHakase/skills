...
正規のTypeと成果の形に応じて、PRマージを必須にするかを変える。固有条件を満たしたIssueを完了候補にし、最後に選択した正本の共通項目を更新する。

| 成果種別           | 固有の完了条件                                                                                                         |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| ブランチ作成型     | PRが既定ブランチへマージ済みで必須検査が成功し、受け入れ条件を確認済み                                                 |
...
- 承認と必須検査が完了し、設定済みの自動・キュー・手動統合を待つ: マージ待ち枠を消費する。
...
- 通常のレビュー待ち、必須検査の実行中、マージ待ちはこのStatusに保つ。
