...
正規のTypeと成果の形に応じて、PRマージを必須にするかを変える。固有条件を満たしたIssueを完了候補にし、最後に選択した正本の共通フィールドを更新する。

| 成果種別           | 固有の完了条件                                                                                                         |
| ------------------ | ---------------------------------------------------------------------------------------------------------------------- |
| ブランチ作成型     | PRが既定ブランチへマージ済みで必須ステータスチェックが成功し、受け入れ条件を確認済み                                   |
...
- マージ待ち枠: 設定済みの統合経路で同時にマージ待ちへ置けるPR数。
...
- 承認と必須ステータスチェックが完了し、設定済みの自動・キュー・手動統合を待つ: マージ待ち枠を消費する。
...
同じ入力、同じフィールド値、同じ容量なら同じ実行Waveになるようにする。手動で順序を変えた場合は理由を計画へ記録する。
...
5. **事後再取得**: Projectアイテム、作業権取得コメント、紐づくブランチ、未完了PRを再取得する。最古コメントが自分で、Agent Runが自分の実行IDと完全一致し、競合ブランチがない場合だけ、リポジトリ差分を作るIssueではブランチと`worktree`を作って実装を始める。ブランチ作成の成功を最終的な排他確認にする。
...
- 解放では作業を停止し、未プッシュの変更の扱いを記録してからAgent Runを空欄にする。紐づくブランチを残すか閉じるかも明記する。
...
- Agent Run、Assignee、Agent Harness、Agent Model、Branch、Actual Start、紐づくブランチをProject/GitHubメタデータへ記録する。
