...
description: >-
  GitHub IssueとPull Requestを作業単位として、仕様や不具合からのIssue分割・起票、Issue FormsとPRテンプレート、sub-issue・blocked by・Milestone、Assignee・linked branch・open PRによる着手競合の停止・引き継ぎ・中断、branchとPRの対応、PR本文、レビューからマージまでの契約を設計・運用・監査するときに使う。stacked PR、PR Stack、`gh stack`による分割・提出・同期・再構成・マージ設計にも使う。GitHub Projectsを使わないリポジトリにも使う。単一Issueや単一PRでも、本文構成、受け入れ条件、確認手順、関連付けを判断して作る依頼には使う。閲覧・要約・担当者変更・翻訳だけ、通常のコードレビュー、CI失敗修正、一般的なGit操作、`gh stack --help`、Stack状態の閲覧、単発コマンドの説明、Projects固有の項目・工数・容量だけには使わない。ProjectsとIssue/PRの両方を扱う依頼ではgithub-project-opsと併用する。
compatibility: GitHub.comと認証済みの現行gh CLIを想定する。Stackを扱う場合はGit 2.20以上、gh 2.90.0以上、gh-stack拡張、同一リポジトリ内のbranchを必要とする。Public Preview中のため、実行時の`gh stack <command> --help`を本文の固定例より優先する。GitHub MCPは利用可能なら探索と状態確認に使う。sub-issue、Issue dependencies、Issue Type、マージキューは対象リポジトリで確認できた機能だけを使い、機能不足、権限不足、確認不能では推測せず停止する。
---
...
- stacked PRの分割、base順、提出、同期、再構成、レビュー、マージ
...
- `gh stack`のヘルプ表示、状態閲覧、単発コマンドの説明だけの依頼
...
- Stack番号、所属PR、順序、baseはGitHubのStackメタデータを正本にする。`.git/gh-stack`はローカル操作用の状態に限り、共有の正本にしない。
...
| stacked PRの設計、提出、同期、再構成、マージ、復旧         | `references/stacked-prs.md`              |
...
- Stack番号、所属PR、下段からの順序、各PRのbase、ローカル追跡との差
...
Stack操作では実行直前に対象サブコマンドの`--help`を読み、対象のbranch、PR、Stack番号とremoteを明示して対話選択へ入らない。`gh stack modify`は対話操作なので自動実行しない。コマンドの選択と停止条件は`references/stacked-prs.md`に従う。
...
flowchart TD
    ...
    structure["通常PRかStackかを決める"]
    implementation["branchと作業場所を用意する"]
    draftPr["Draft PRを作成・提出する"]
...
Stackの標準方式だけは、Stack全体を1人の調整担当と1つの専用`worktree`で扱う。同じStackのbranchを別`worktree`で開かない。複数`worktree`方式ではローカル追跡を共有せず、各末端Issueの担当者と調整担当の直列引き継ぎを`references/stacked-prs.md`どおりに行う。
...
- 1つのStackは1つの`epic`に所属させ、1段を「1末端Issue = 1 branch = 1 PR」に対応させる。各PRは対応する末端Issueだけを閉じ、`epic`を閉じない。
- `epic` / sub-issueは成果の階層、blocked by / blockingは着手不能の依存、Stackのbase関係はbranch・レビュー・マージの直列順序として別々に扱う。Stackの隣接関係をblocked byへ自動変換しない。
- 1つの`epic`は複数Stackや通常PRを含めてよいが、1つのStackを複数`epic`へまたがせない。独立して並行できる変更は通常PRまたは別Stackに分ける。
...
- Stack: マージ対象として選んだ最下段からの連続範囲が`gh stack merge`で完了し、対応する末端Issueが閉じている。残る上段と`epic`は個別に判定する。
